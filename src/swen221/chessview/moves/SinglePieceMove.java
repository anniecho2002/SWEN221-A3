...
public class SinglePieceMove implements MultiPieceMove {
	...
	public void apply(Board b) {
		...
		//System.out.println("OLD: " + oldPosition.toString() + "  NEW: " + newPosition.toString());
		this.toString();
		if(piece instanceof King) {
			...
			if(piece.isWhite()) {
				if(oldPosition.equals(new Position(1,1))) { b.setRookMoved(true,false); }
				else if(oldPosition.equals(new Position(1,8))) { b.setRookMoved(true,true); }
			} 
			else {
				if(oldPosition.equals(new Position(8,1))) { b.setRookMoved(false,false); }
				else if(oldPosition.equals(new Position(8,8))) { b.setRookMoved(false,true); }
			}
		}
		else if(piece instanceof Pawn) {
			// Store the double-step information required to implement EnPassant
