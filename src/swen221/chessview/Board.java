...
public class Board {
	...
	/**
	 * Check whether one of the rooks has moved.
	 *
		...
		outer: for (int row = 1; row <= 8; ++row) {
			for (int col = 1; col <= 8; ++col) {
				...
				if (p instanceof King && p.isWhite() == isWhite) {
					// found him.
					kingPos = pos;
					king = (King)this.pieceAt(kingPos);
	...
	/**
	 * Finds all the position of a color's pieces
	 * Checks if the king of the color passed in the parameter is in check mate
	 * 
	 */
	public boolean isCheckmate(boolean isWhite) {
		
		ArrayList<Position> pieces = new ArrayList<Position>(); // holds the positions of all the pieces on the board
		
		// Go through positions of all the pieces on the board
		for (int row = 1; row <= 8; ++row) {
			for (int col = 1; col <= 8; ++col) {
				...
				// if there actually is a piece in that position, and it's of the right color
				if(p != null && p.isWhite() == isWhite) {
					pieces.add(pos); // add it into the array of pieces
				}
			}
		}

		// Check all the pieces to see where it is moving to
		for (int row = 1; row <= 8; ++row) {
			for (int col = 1; col <= 8; ++col) {
				
				Position toPos = new Position(row, col);
				Piece p = pieceAt(toPos);
				for(Position listPos: pieces) {
					Piece p2 = pieceAt(listPos);
					if(p2.isValidMove(listPos, toPos, p, this)) {
						Board copy = new Board(this);
						copy.move(listPos, toPos);
						if(!copy.isInCheck(isWhite) && (p2==null) || (p.isWhite() != p2.isWhite())) {
							return true;
						}
					}
				}
	...
	/**
	 * The following method checks whether the given diagonal is completely
	 * clear, except for a given set of pieces. Observe that this doesn't
	 * guarantee a given diagonal move is valid, since this method does not
	 * ensure anything about the relative positions of the given pieces.
	 *
	 * @param startPosition - start of diagonal
	 * @param endPosition - end of diagonal
	 * @param exceptions - the list of pieces allowed on the diagonal
	 * @return True if the diagonal is clear.
	 */
	public boolean clearDiagonalExcept(Position startPosition,
